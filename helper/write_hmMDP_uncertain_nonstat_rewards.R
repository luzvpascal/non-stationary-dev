write_hmMDP_uncertain_nonstat_rewards <- function(TR_FUNCTION,
                                                  TR_FUNCTION_REW,
                                                  TR_FUNCTION_TIMES,
                                                  B_FULL,
                                                  B_FULL_REW,
                                                  ...
                                                  GAMMA,
                                                  FILE){
  # TR_FUNCTION: list of arrays of size [s,s,a] for the states (N_models=length(TR_FUNCTION)) independent of time
  # TR_FUNCTION_REW: list of arrays of size [s*T, num_rew, a] (N_models_rew=length(TR_FUNCTION_REW)) observation function
  # TR_FUNCTION_TIMES: matrix of size [T, T]. transition function between time steps
  ...
  # B_FULL_TIMES: vector, probability distribution over the fully observable state(time step)
  ...
  header_belief <- paste0("<InitialStateBelief>\r\n\n",
                          ...
                          "<Entry>\r\n",
                          "<Instance> - </Instance>\r\n",
                          "<ProbTable>", paste0(B_FULL_TIMES, collapse = " "), "</ProbTable>\n",
                          "</Entry>\r\n",
                          "</Parameter>\r\n",
                          "</CondProb>\r\n\n",
  ...
  rew_header <- paste0(
    ...
    "<Parent>action_control times_0 hidden_rew_0 species_0</Parent>\r\n",
    "<Parameter type=\"TBL\">\r\n"
  )
  for (act_id in seq(Num_a)){
    action_text <- ACTIONS[act_id]
    ...
    for (mod0_rew_id in seq(Num_mod_rew)){
      ...
      for (state0_id in seq(Num_s)){
        ...
        rew_header <- paste(rew_header,
                            ...
                            as.character(REW[[mod0_rew_id]][tuple_to_index(time0_id,state0_id),act_id]),
                            "</ValueTable>\r\n",
                            "</Entry>\r\n")
      }
    }
