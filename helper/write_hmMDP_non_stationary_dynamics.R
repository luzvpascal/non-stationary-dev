write_hmMDP_non_stationary_dynamics <- function(TR_FUNCTION,
  ...
  tr_models <-paste("<CondProb>\r\n",
                    "<Var>hidden_1</Var>\r\n",
                    "<Parent>action_control hidden_0</Parent>\r\n",
                    "<Parameter type=\"TBL\">\r\n")
  for (act_id in seq(Num_a)){
    action_text <- ACTIONS[act_id]
    action_prob_values <- TR_FUNCTION_MODELS[,,act_id]
    for (mod0_id in seq(Num_mod)){
      model0_text <- MODELS[mod0_id]
      for (mod1_id in seq(Num_mod)){
        model1_text <- MODELS[mod1_id]
        tr_models <- paste(tr_models,
         "<Entry>\r\n",
         "<Instance>",
         action_text, model0_text, model1_text,
         " </Instance>\r\n",
         "<ProbTable>",
         as.character(action_prob_values[mod0_id,mod1_id]),
         "</ProbTable>\r\n",
         "</Entry>\r\n")
      }
    }
